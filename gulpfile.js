var cwd = process.cwd(),
    ...
    paths = {
      ...
      scripts: {
        in: 'src/js',
        out: 'public/js',
        lintFolders: [ 'footer', 'google-maps-kickstart' ]
      },
      images: {
        in: 'src/images/**/*',
        out: 'public/images'
...
gulp.task('cssstyles', function() {
  return gulp.src( paths.css.in )
    ...
    .pipe( cssnano( {autoprefixer: false} ) )
    .pipe( gulp.dest( paths.css.out ) )
    .pipe( notify( { message: 'CSS Only Styles task complete', onLast: true } ) );
});
...
gulp.task('watch', function() {
  ...
  gulp.watch( 'src/css/*.css', ['cssstyles'] );
  gulp.watch( ['src/css/*.*', "!src/css/*.css"], ['move'] );
